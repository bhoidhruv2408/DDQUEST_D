// tests.js – Semester 4 Tests (waits for Firebase)
...
async function loadTests() {
    ...
    console.log('📥 Fetching all tests (will filter for semester 4)...');
    ...
    try {
        ...
        const filteredBySemester = all.filter(t => {
            ...
            const isSem4 = sem == 4;
            if (!isSem4) {
                console.log(`🚫 Excluding test "${t.title}" (semester: ${sem}) – not semester 4`);
            }
            return isSem4;
        });
        ...
        if (filtered.length === 0) {
            testsGrid.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Check back later for semester 4 tests.</p>
                </div>
            `;
            return;
        }
