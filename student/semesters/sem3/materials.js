// materials.js – Semester 3 Materials with strict filtering by semester and branch
...
async function loadMaterials() {
    ...
    console.log('📥 Fetching all materials (will filter for semester 3)...');
    ...
    try {
        ...
        const filteredBySemester = all.filter(m => {
            ...
            if (!isSem3) {
                console.log(`🚫 Excluding material "${m.title}" (semester: ${sem}) – not semester 3`);
            }
            return isSem3;
        });
        ...
        // Filter by branch: include if material has no branch OR branch matches
        ...
        if (filtered.length === 0) {
            materialsGrid.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Check back later for semester 3 study materials.</p>
                </div>
            `;
            return;
        }
...
// Online status, time, greeting, particles, etc. (same as sem2 materials.js)
