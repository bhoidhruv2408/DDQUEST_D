...
async function loadUpcomingTests() {
    ...
    try {
        ...
        // Check if it's the index error and extract the link
        if (error.code === 'failed-precondition') {
            const match = error.message.match(/https:\/\/console\.firebase\.google\.com[^\s]+/);
            const indexUrl = match ? match[0] : '#';
            document.getElementById('upcomingTests').innerHTML = `
                <div class="empty-state">
                    <i class="fas fa-database"></i>
                    <h3>Index Required</h3>
                    <p>To see upcoming tests, you need to create a Firestore index.</p>
                    <p><a href="${indexUrl}" target="_blank" style="color: var(--accent);">Click here to create the index</a></p>
                    <p style="font-size: 0.8rem; margin-top: 10px;">After creating, wait a minute and refresh.</p>
                </div>
            `;
        } else {
