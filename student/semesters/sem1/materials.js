// materials.js – Semester 1 Materials with strict filtering by semester and branch
...
async function loadMaterials() {
    ...
    console.log('📥 Fetching all materials (will filter for semester 1)...');
    ...
    try {
        ...
        // Filter by semester 1 – strict equality, handle both number and string
        const filteredBySemester = all.filter(m => {
            const sem = m.semester;
            // Allow both number 1 and string "1"
            const isSem1 = sem == 1; // loose equality
            if (!isSem1) {
                console.log(`🚫 Excluding material "${m.title}" (semester: ${sem}) – not semester 1`);
            }
            return isSem1;
        });
        ...
        if (filtered.length === 0) {
            materialsGrid.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Check back later for semester 1 study materials.</p>
                </div>
            `;
            return;
        }
